...
export const getLevelByPoints = (points: number) => {
  return SUSHI_LEVELS.slice().reverse().find(l => points >= l.pointsRequired) || SUSHI_LEVELS[0];
};

export const getNextLevel = (points: number) => {
  return SUSHI_LEVELS.find(l => l.pointsRequired > points) || null;
};
...
// Recalculate level if saved user has lifetimePoints
if (initialUser && initialUser.lifetimePoints !== undefined) {
  const levelInfo = getLevelByPoints(initialUser.lifetimePoints);
  const nextLevel = getNextLevel(initialUser.lifetimePoints);
  initialUser.level = levelInfo.name;
  initialUser.nextLevelPoints = nextLevel ? nextLevel.pointsRequired : levelInfo.pointsRequired;
  // Update localStorage with corrected values
  localStorage.setItem('hoffee_user', JSON.stringify(initialUser));
}
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  setAuth: (user) => {
    ...
    const nextLevelPoints = nextLevel ? nextLevel.pointsRequired : levelInfo.pointsRequired;

    const finalUser = {
      ...user,
      level,
      nextLevelPoints,
      // Ensure these are never empty
