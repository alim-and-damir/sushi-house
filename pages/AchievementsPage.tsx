...
import { useAppStore, SUSHI_LEVELS } from '../store/useAppStore';
...
export const AchievementsPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] pb-32">
      ...
      <div className="p-6">
        <div className="bg-[#2F4A5A] rounded-[2.5rem] p-8 text-white mb-8 shadow-2xl relative overflow-hidden animate-slide-down">
          ...
          <div className="relative z-10 flex flex-col items-center">
            <div className="w-20 h-20 bg-white/10 backdrop-blur-md rounded-3xl flex items-center justify-center mb-4 border border-white/20 animate-pop-in">
              <Trophy size={40} className="text-[#D6B24A]" />
            </div>
            <p className="text-white/80 text-[10px] font-black uppercase tracking-[0.2em] mb-2">Ваш текущий ранг</p>
            ...
            <div className="w-full bg-black/20 h-3 rounded-full overflow-hidden mb-3 border border-white/5">
              <div
                className="h-full rounded-full shadow-[0_0_10px_rgba(186,151,95,0.5)] transition-all duration-1000 ease-out"
                style={{
                  width: `${Math.min(100, (currentPoints / user.nextLevelPoints) * 100)}%`,
                  background: 'linear-gradient(to right, #D6B24A, #d4bc8e)'
                }}
              />
            </div>
            <div className="flex items-center gap-2">
               <span className="text-sm font-bold text-white">{currentPoints}</span>
               <span className="text-xs text-[#D6B24A]/60">/</span>
               <span className="text-xs text-[#D6B24A]/60 font-medium">{user.nextLevelPoints} баллов за все время</span>
            </div>
          </div>
        </div>

        <div className="bg-white rounded-3xl p-6 mb-10 shadow-sm border-2 border-[#2F4A5A] animate-slide-up stagger-1 opacity-0 fill-mode-forwards">
          <div className="flex items-center gap-2 mb-4">
            <span className="text-[#2F4A5A]">
              <Info size={18} />
            </span>
            <h3 className="font-black uppercase text-xs tracking-wider text-slate-900">Как заработать баллы?</h3>
          </div>
          <div className="space-y-4">
            <div className="flex items-start gap-4">
              <div className="w-8 h-8 bg-[#2F4A5A]/10 rounded-xl flex items-center justify-center shrink-0">
                <Coins size={16} className="text-[#2F4A5A]" />
              </div>
              <p className="text-sm font-medium leading-snug text-slate-700">Получайте <span className="font-bold text-[#2F4A5A]">5% кэшбэка</span> баллами при полной оплате рублями.</p>
            </div>
            <div className="flex items-start gap-4">
              <div className="w-8 h-8 bg-[#2F4A5A]/10 rounded-xl flex items-center justify-center shrink-0">
                <Share2 size={16} className="text-[#2F4A5A]" />
              </div>
              <p className="text-sm font-medium leading-snug text-slate-700">Подписывайтесь на наши соцсети и получайте <span className="font-bold text-[#2F4A5A]">50 баллов</span> единоразово!</p>
            </div>
            <div className="flex items-start gap-4">
              <div className="w-8 h-8 bg-[#2F4A5A]/10 rounded-xl flex items-center justify-center shrink-0">
                <UserPlus size={16} className="text-[#2F4A5A]" />
              </div>
              <p className="text-sm font-medium leading-snug text-slate-700">Дарим <span className="font-bold text-[#2F4A5A]">100 баллов</span> за каждого друга, который совершит первую покупку.</p>
            </div>
          </div>
        </div>
            ...
            return (
              <div
                ...
                className={`bg-white rounded-3xl p-5 border transition-all duration-500 animate-slide-up ${staggerClass} opacity-0 fill-mode-forwards ${
                  isCurrent
                    ? 'ring-2 ring-[#2F4A5A] border-transparent shadow-md z-10'
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        {isCurrent && (
                          <span className="text-[9px] font-bold text-[#2F4A5A] bg-[#2F4A5A]/10 px-2 py-0.5 rounded-full">
                            Текущий
                          </span>
                        )}
                      </div>
