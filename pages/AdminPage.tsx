...
export const AdminPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-24 relative overflow-x-hidden">
      {/* Header */}
      <div className="bg-[#2F4A5A] text-white p-6 rounded-b-[2.5rem] shadow-lg mb-4 animate-fade-in">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[#D6B24A]/80 text-xs">Администрирование Sushi House</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4">
        ...
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#2F4A5A]/10 text-[#2F4A5A] rounded-full flex items-center justify-center">
        ...
        <div className="relative flex gap-1 bg-white p-1 rounded-2xl border border-slate-200 mb-6">
          <div 
            className="absolute top-1 bottom-1 bg-[#2F4A5A] rounded-xl transition-all duration-300 ease-out shadow-lg"
            style={{ 
              width: 'calc(50% - 2px)', 
        ...
        {activeTab === 'menu' && (
          <div className="space-y-4 animate-slide-up">
            ...
            {lastOperation && (
              <div className="bg-white border border-[#2F4A5A]/20 p-5 rounded-[2rem] shadow-sm mb-6 animate-pop-in overflow-hidden relative group">
                {/* Decorative background accent */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-[#2F4A5A]/10 rounded-full -mr-12 -mt-12 transition-transform group-hover:scale-110 duration-500" />

                <div className="flex items-center gap-2 text-[#2F4A5A] mb-4 relative z-10">
                  <History size={16} />
                        ...
                        <div className="mt-2 space-y-1 bg-slate-50/50 p-2 rounded-xl border border-slate-100">
                          {lastOperation.changes.map((ch, i) => (
                            <div key={i} className="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold uppercase tracking-tight">
                              ...
                              <span className="text-[#2F4A5A]">{ch.to}</span>
                            </div>
                          ))}
                        </div>
                      )}
                ...
                <div className="pt-4 border-t border-slate-50 relative z-10">
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-[#2F4A5A] text-white py-3 rounded-xl text-xs font-black uppercase tracking-widest active:scale-[0.98] transition-all shadow-lg shadow-[#2F4A5A]/30"
                  >
                    <Undo2 size={14} />
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <button 
                ...
                className="bg-[#2F4A5A] text-white p-2 rounded-xl shadow-lg active:scale-90 transition-all"
              >
                <Plus size={20} />
              </button>
            </div>
            {products.map((p) => (
              <div key={p.id} className="bg-white p-4 rounded-3xl shadow-sm flex items-center gap-4 border border-slate-100">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleEditProduct(p)} className="p-2 bg-slate-50 text-slate-500 rounded-lg hover:bg-[#2F4A5A]/10 hover:text-[#2F4A5A] transition-colors">
                    <Edit3 size={18} />
                  </button>
          ...
          <div className="space-y-6 animate-slide-up">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
                <div className="w-10 h-10 bg-[#2F4A5A]/10 text-[#2F4A5A] rounded-xl flex items-center justify-center mb-3">
                  <ShoppingBag size={20} />
                </div>
            ...
            <div className="bg-[#2F4A5A] text-white p-6 rounded-3xl shadow-lg">
              <h3 className="font-bold mb-2">Аналитика недели</h3>
              <p className="text-[#D6B24A]/70 text-xs mb-4">Статистика продаж по категориям</p>
                ...
                {categories.map(cat => (
                  <div key={cat.id}>
                    ...
                    <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div className="h-full bg-[#D6B24A] rounded-full" style={{ width: `${Math.floor(Math.random() * 80) + 20}%` }} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center animate-fade-in">
          ...
          <div className="bg-white w-full max-w-lg rounded-t-[2.5rem] p-6 pb-12 relative z-[101] shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              <div className="flex gap-4">
                ...
                <div className="flex-1 space-y-3">
                  <input 
                    ...
                    className="w-full bg-slate-50 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-[#2F4A5A]"
              ...
              <button 
                onClick={saveProduct}
                className="w-full bg-[#2F4A5A] text-white py-4 rounded-2xl font-bold shadow-lg shadow-[#2F4A5A]/30 flex items-center justify-center gap-2 active:scale-95 transition-all"
              >
                <Save size={20} />
                Сохранить изменения
              </button>
